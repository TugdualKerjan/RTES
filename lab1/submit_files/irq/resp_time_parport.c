...
int main()
{
    ...
    IOWR_8DIRECT(PAR_PORT_0_BASE, PARIRQEN, ALL_IRQ_DIS);  // Clear interrupt
    IOWR_8DIRECT(PAR_PORT_0_BASE, IREGPORT, 0x00);
    IOWR_8DIRECT(PAR_PORT_0_BASE, PARIRQEN, ALL_IRQ_EN); // Enable IRQ on each bit
    while (1)
    {
        IOWR_8DIRECT(PAR_PORT_0_BASE, IREGPORT, 0x02); // Bit2 is SET st IRQ generated
        // if (flag)
        // {
        //     alt_printf("INTERRUPT!\n");
        //     // IOWR_16DIRECT(NIOS_LEDS_BASE, 0, (1 << 10) - 1);
        //     usleep(2 * SLEEP_DELAY_US);
        //     // IOWR_16DIRECT(NIOS_LEDS_BASE, 0, 0);
        //     // usleep(2 * SLEEP_DELAY_US);
        //     // IOWR_8DIRECT(PAR_PORT_0_BASE, PARIRQEN, ALL_IRQ_EN); // Enable interrupt
        //     flag=0;
        // }
    }
    return 0;
...
static void my_isr(void *context)
{
    ...
    IOWR_8DIRECT(PAR_PORT_0_BASE, PARIRQEN, ALL_IRQ_EN);  // Clear interrupt
    // flag = 1;
}
