...
END ENTITY accel_averager;

ARCHITECTURE comp OF accel_averager IS
    ...
    SIGNAL iPixelInput : STD_LOGIC_VECTOR(31 DOWNTO 0);
        ...
        ELSIF rising_edge(clk) THEN
            ...
            CASE StateM IS
                ...
                WHEN WAITR => -- wait for value
                    iPixelInput <= (OTHERS => '0');
                    IF mWaitRequest = '0' THEN
                        iPixelInput <= mReadData;
                        mRead <= '0';
                        StateM <= COMPUTE;
                    END IF;
                WHEN COMPUTE => -- swap
                    iRedAggregator <= iRedAggregator + unsigned(iPixelInput(7 DOWNTO 0));
                    iGreenAggregator <= iGreenAggregator + unsigned(iPixelInput(15 DOWNTO 8));
						  iBlueAggregator <= iBlueAggregator + unsigned(iPixelInput(23 DOWNTO 16));
						  -- highest 8 bits just for alignment
                    iIndex <= iIndex + 1;
                    StateM <= CHECK;
                ...
                WHEN WRITE_WD => -- write output word
                    ...
                    mWriteData(23 DOWNTO 16) <= STD_LOGIC_VECTOR(iBlueAggregator(7 DOWNTO 0));
						  mWriteData(31 downto 24) <= (others => '0');
                    StateM <= WAITW;
